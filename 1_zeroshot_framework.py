...
class RunInference:
    ...
    def _save_results(self, current_batch_df):
        if not os.path.exists(f"codeswitched_results/{self.model_name}/zero-shot"):
            os.makedirs(f"codeswitched_results/{self.model_name}/zero-shot")

        file_name = os.path.basename(self.data_path)
        file_name_without_ext = os.path.splitext(file_name)[0]

        output_path = f"codeswitched_results/{self.model_name}/zero-shot/{self.dataset_name}_{file_name_without_ext}_processed.tsv"
